...
/**
  * ScalaCreateStreamingGraphs
  *
  * Author: Noprom <tyee.noprom@qq.com>
  * Date: 2/26/16 3:27 PM.
  */
  ...
  def main(args: Array[String]) {
    ...
    val graphStream = newDstream.foreachRDD { x =>
      ...
      val vertices:RDD[(VertexId, (String))] =
        sparkCtx.parallelize(tuple._1.toSeq)
      // Define Edges
      ...
      val filteredRDD = graph.vertices.filter ( x =>
        x._2.startsWith("Request=")).map(x => (x._2, 1)).reduceByKey(_+_)
